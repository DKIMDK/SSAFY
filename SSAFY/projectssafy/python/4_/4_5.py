...
def create_user(users_data):
    ...
    for user_dict in users_data:
        is_valid = is_validation(user_dict)
        # print(is_valid)      
        # if is_valid == 'blocked':
        #     err_cnt += 1
    ...
    print(f'잘못된 데이터로 구성된 유저의 수는 {err_cnt} 입니다.')
    return user_list



def is_validation(user_dict):
    validation = True
    wrong_data = []
    if not user_dict.get('blood_group') in blood_types:
        validation = False
        wrong_data.append('blood_group')
    elif '@' not in user_dict.get('mail'):
        validation = False
        wrong_data.append('mail')
    elif len(user_dict.get('name')) < 2 or len(user_dict.get('name')) > 30 :
        validation = False
        wrong_data.append('name')
    elif not len(user_dict.get('website')) > 0 :
        validation = False
        wrong_data.append('website')
    elif user_dict.get('company') in black_list:
        validation = False
        return validation, 'blocked'
    if not validation:
        # print(wrong_data)
        return validation, wrong_data
    return validation, wrong_data
