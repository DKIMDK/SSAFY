...
SELECT 소속회사, 이름, 나이 FROM superhero
...
-- 소속회사, 나이 순으로 정렬
SELECT DISTINCT 이름, 나이, 소속회사 FROM superhero
ORDER BY 소속회사, 나이;
...
SELECT * FROM superhero WHERE 국적 IN ('미국', '러시아'); 
----------------------
...
SELECT DISTINCT 소속회사, AVG(나이) AS 평균나이 from superhero
WHERE 소속회사 = '마블';
...
SELECT 소속회사, AVG(나이) FROM superhero
WHERE 나이 >= 40
GROUP BY 소속회사;
-- 그룹화 후에 조건주기(HAVING)
-- 4.11 평균 나이가 40살 이상인 소속 회사를 구하라
SELECT 소속회사, AVG(나이) FROM superhero
GROUP BY 소속회사 HAVING AVG(나이) >= 40;