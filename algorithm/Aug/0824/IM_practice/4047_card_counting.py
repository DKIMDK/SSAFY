...
def solution(cards):
    sdhc = [[0] * 14 for _ in range(4)]
    for card in cards:
        if card[0] == 'S':
            sdhc[0][int(card[1:2 + 1])] += 1
        elif card[0] == 'D':
            sdhc[1][int(card[1:2 + 1])] += 1
        elif card[0] == 'H':
            sdhc[2][int(card[1:2 + 1])] += 1
        elif card[0] == 'C':
            sdhc[3][int(card[1:2 + 1])] += 1
    for suit in sdhc:
        if any(value > 1 for value in suit[1:]):
            return ['ERROR']
    return [13-sum(sdhc[0]), 13-sum(sdhc[1]), 13-sum(sdhc[2]), 13-sum(sdhc[3])]
...
#     card = input()
#     check = []
#     card_dict = {'S' : 13, 'D' : 13, 'H' : 13, 'C' : 13}     #card left
...
#             num = card_dict[card[i]] - 1    # 해당 무늬 카드 수 1개 차감
