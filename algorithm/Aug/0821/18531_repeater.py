...
2. 좌표는 좌측 상단부터 (0, 0)으로 시작한다. 지도의 크기는 N x N 크기의 정사각형이며, 모든 집들의 좌표는 격자점 위에 있으므로,
입력으로 주어지는 2차원 Map Data의 크기는 (N+1)^2이 됨을 유의하라.
...
def solution(arr):
    ...
    for y in range(N):
        for x in range(N):
            if arr[y][x] == 2:
                repeater_y, repeater_x = y, x
    for y in range(N):
        for x in range(N):
            if arr[y][x] == 1:
                square_d = (repeater_y - y) ** 2 + (repeater_x - x) ** 2
                if max_d <= square_d:
                    max_d = square_d
    while r ** 2 < max_d:
        r += 1
